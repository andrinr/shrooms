...
Colors adapt to the selected species’ scores among tiles whose centers are in the visible map window: the 10th percentile is the low color endpoint and the 95th percentile is the high endpoint. Scores outside that interval clamp to the endpoint colors. Pink highlights the strongest relative signals, while weaker cells are more transparent. The legend displays the actual score thresholds. Endpoints update after panning or zooming, and when species or weather changes. In overview mode the scale uses displayed group means; in detail mode it uses individual cell scores. Offscreen render-buffer tiles do not affect the endpoints. A minimum 10-point span prevents tiny differences from being stretched across the entire palette. Numeric suitability scores are unchanged; these colors do not indicate calibrated probabilities or guarantee good conditions.
